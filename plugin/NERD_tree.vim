...
call s:initVariable("g:NERDTreeMapChangeRootAndCwd", "d")
...
function! s:dumpHelp()
    ...
    if b:treeShowHelp ==# 1
        ...
        let @h=@h."\" ". g:NERDTreeMapChangeRootAndCwd .": change tree root and cwd to the\n"
        let @h=@h."\"    selected dir\n"
...
function! s:toggle(dir)
    if s:treeExistsForTab()
        if !s:isTreeOpen()
            ...
            " gukz resize the tree
            execute "vert resize " . g:NERDTreeWinSize
        else
            call s:closeTree()
        endif
    else
...
function! s:bindMappings()
    ...
    exec "nnoremap <silent> <buffer> ". g:NERDTreeMapChangeRootAndCwd ." :call <SID>chRootAndCwd()<cr>"
...
" FUNCTION: s:chRootAndCwd() {{{2
" changes the current root and cwd to the selected one
function! s:chRootAndCwd()
    call s:chRoot()
    " gukz
